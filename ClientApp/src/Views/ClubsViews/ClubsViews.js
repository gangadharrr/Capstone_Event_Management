...
export function ClubsPresidentView() {
    const navigate = useNavigate()
    const [data, setData] = useState(null)
    const [spinner, setSpinner] = useState(true);
    useEffect(() => {
        authService.getAccessToken().then(token => {
            authService.getUser().then(user => {
                axios.get(`customidentityrole/${user.name}/1`, 
                {
                    ...
                    axios.get(`clubs/${response.data.email}/1`, {
                        headers: !token ? {} : { 'Authorization': `Bearer ${token}` }
                    }).then((response) => {
                        setData(response.data)
                        setSpinner(false)
...
export function ClubsPresidentEditView() {
    const location = useLocation()
    const [spinner, setSpinner] = useState(true);
    const queryParameters = new URLSearchParams(location.search)
    const navigate = useNavigate()
                        ...
                        <React.Fragment>
                            ...
                            <div className="row" key={data.clubId}>
                                <div className="col-md-4">
                                    ...
                                    <div className="form-group">
                                        <button className="btn btn-success" onClick={submitForm}>Save</button>&nbsp;&nbsp;
                                        <Link className="btn btn-warning" to={"/clubs-president-view"}>Back to List</Link>
                                    </div>
                                </div>
                            </div>
                        </React.Fragment>
...
export function ClubsPresidentDetailsView() {
    const location = useLocation()
    const [spinner, setSpinner] = useState(true);
    const [data, setData] = useState(null)
    const queryParameters = new URLSearchParams(location.search)
    ...
    return (
        <React.Fragment>
            ...
            {spinner ? <LoadingAnimation type='fallinglines' text="Loading..." /> : <React.Fragment>
                ...
                <div>
                    <Link className='btn btn-warning' to={'/clubs-president-edit-view?id=' + data.clubId}>Edit</Link>&nbsp;
